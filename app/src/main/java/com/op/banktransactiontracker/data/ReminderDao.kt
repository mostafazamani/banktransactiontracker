...
interface ReminderDao {
    ...
    @Query("SELECT * FROM reminders WHERE type = :type AND status = 'ACTIVE'")
    suspend fun getAllByType(type: ReminderType): List<ReminderEntity>

    @Query("SELECT * FROM reminders WHERE reminderDate >= :start AND reminderDate <= :end AND status = 'ACTIVE'")
